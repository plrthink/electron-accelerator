module.exports = function (grunt) {
  ...
  grunt.registerTask('debug-app', 'Starts an electron app ready to debug with node inspector', function (buildOption) {
    ...
    var debuggerProcess, electronProcess

    if (process.platform === 'win32') {
      debuggerProcess = proc.exec('node-inspector', ['--web-port=1337'])
      electronProcess = proc.spawn(electron, ['--debug=5858', '.'])
      open('http://127.0.0.1:1337/?ws=127.0.0.1:8080&port=5858', 'chrome')
    } else {
      debuggerProcess = proc.spawn('node-inspector', ['--web-port=1337'])
      electronProcess = proc.spawn(electron, ['--debug=5858', '.'])
      open('http://127.0.0.1:1337/?ws=127.0.0.1:8080&port=5858', 'google chrome')
    }
